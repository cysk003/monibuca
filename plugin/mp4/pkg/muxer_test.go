...
func TestFLVToFMP4(t *testing.T) {
	...
	// Create FMP4 muxer
	muxer := NewFMP4Muxer(outFile)
	muxer.SetFragmentDuration(2000) // 2 seconds per fragment
			...
			if codecID == 7 { // AVC/H.264
				if tag.Data[1] == 0 { // AVC sequence header
					...
					sample := box.Sample{
						...
						Duration: uint32(tag.Timestamp - lastVideoTime),
						KeyFrame: frameType == 1,
					}
					if err := muxer.WriteSample(videoTrack, sample); err != nil {
						t.Fatalf("Failed to write video sample: %v", err)
					}
					lastVideoTime = tag.Timestamp
					frameCount++
